document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function (e) {
      ...
      if (!nameVal || !emailVal || !msgVal) {
        showFormMsg('Please fill in all fields.', 'error', btn);
        return;
      }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailVal)) {
        showFormMsg('Please enter a valid email.', 'error', btn);
        return;
      }
      ...
      try {
        ...
        formData.append('subject', 'New Portfolio Contact — ' + nameVal);

        const res    = await fetch('https://api.web3forms.com/submit', { method: 'POST', body: formData });
        const result = await res.json();

        if (result.success) {
          showFormMsg('Message sent! I\'ll get back to you soon. 🚀', 'success', btn);
          contactForm.reset();
        } else {
          throw new Error(result.message || 'Submission failed');
        }
      } catch (err) {
        showFormMsg('Oops! Something went wrong. Email me directly at ghl.expert99@gmail.com', 'error', btn);
      } finally {
        btn.disabled  = false;
        lucide.createIcons();
        btn.innerHTML = '<span>SEND MESSAGE</span><i data-lucide="send" size="20"></i>';
        lucide.createIcons();
      }
    });
  }
  ...
  function showFormMsg(msg, type, btn) {
    ...
    toast.className = 'form-msg-toast';
    toast.style.cssText = `
